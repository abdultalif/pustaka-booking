...
class Pinjam extends CI_Controller
{
    public function __construct()
    {
        ...
        cek_login();
        cek_user();
        $this->load->model(['ModelBooking', 'ModelPinjam']);
    }
    ...
    public function daftarBooking()
    {
        ...
        $data['user'] = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();
        $data['pinjam'] = $this->ModelBooking->daftrbooking()->result_array();
        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
    ...
    public function pinjamAct()
    {
        ...
        $this->db->query("UPDATE buku, detail_pinjam SET buku.dipinjam = buku.dipinjam+1, buku.dibooking = buku.dibooking-1 WHERE buku.id_buku = detail_pinjam.id_buku");
    ...
    public function ubahStatus()
    {
        ...
        $this->db->query("UPDATE buku, detail_pinjam SET buku.dipinjam=buku.dipinjam-1, buku.stok=buku.stok+1 WHERE buku.id_buku=detail_pinjam.id_buku");
